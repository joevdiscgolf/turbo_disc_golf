...
class _RoastTabState extends State<RoastTab> {
  ...
  String _buildRoastPrompt() {
    ...
    if (shouldGlaze) {
      buffer.writeln('You are GLAZING someone about their disc golf round (giving excessive, over-the-top compliments in a funny way). Talk TO them, not about them.');
      buffer.writeln('Write a hilarious glaze-up speaking directly to the player - make them sound like a disc golf god!');
    } else {
    ...
    if (shouldGlaze) {
      buffer.writeln('- Reference the specific numbers in ridiculously over-the-top ways (e.g., "bro you hit fairway so much the trees are filing restraining orders against you")');
      buffer.writeln('- Use disc golf insider references to praise them (chains feared you, hyzer flips bowed down, trees parted like the Red Sea, etc.)');
      buffer.writeln('- Mix short punchy compliments with longer over-the-top praise');
      buffer.writeln('- End with an absurdly exaggerated compliment directed at them');
      buffer.writeln('- Make even mediocre stats sound legendary - be hilariously excessive!');
      buffer.writeln('- IMPORTANT: Use **bold** (double asterisks) for emphasis, NOT *italics* (single asterisks) - italics are hard to read!');
    } else {
    ...
    if (shouldGlaze) {
      ...
      buffer.writeln('Remember: be absurdly over-the-top with the compliments. Make them sound like a disc golf legend!');
    } else {
