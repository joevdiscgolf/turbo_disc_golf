...
class RecordRoundPanel extends StatefulWidget {
  const RecordRoundPanel({super.key, required this.bottomViewPadding});
...
class _RecordRoundPanelState extends State<RecordRoundPanel> {
  ...
  void initState() {
    ...
    _transcriptController.addListener(_onTranscriptControllerChange);
  ...
  void _onTranscriptControllerChange() {
    // Sync manual edits back to voice service when user is editing
    // This ensures edits persist when panel is reopened
    if (!_voiceService.isListening && _transcriptFocusNode.hasFocus) {
      _voiceService.updateText(_transcriptController.text);
    }
  }
  ...
  void dispose() {
    ...
    _transcriptController.removeListener(_onTranscriptControllerChange);
